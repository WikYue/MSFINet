...
class MHCA(nn.Module):
    ...
    def forward(self, x):
        ...
        out = out.permute(0,2,3,1).view(B,n_patch,hidden)
...
class Mlp(nn.Module):
    def __init__(self,config, in_channel, mlp_channel):
        ...
        #增加DWconv
        self.dwconv = DWConv(mlp_channel)
        #
...
class Block_ViT(nn.Module):
    def __init__(self, config, vis, channel_num):
        ...
        path_dropout = config.transformer.embeddings_dropout_rate
        ...
        self.mhca1 = MHCA(channel_num[0], head_dim)
        self.mhca2= MHCA(channel_num[1], head_dim)
        self.mhca3= MHCA(channel_num[2], head_dim)
        self.mhca4= MHCA(channel_num[3], head_dim)
...
class ChannelTransformer(nn.Module):
    def __init__(self, config, vis, img_size, channel_num=[64, 128, 256, 512]):
        super().__init__()

        self.patchSize_1 = config.patch_sizes[0]
        self.patchSize_2 = config.patch_sizes[1]
        self.patchSize_3 = config.patch_sizes[2]
        self.patchSize_4 = config.patch_sizes[3]
...
def get_CTranS_config():
    ...
    # config.KV_size = [64,128,320,512]  # KV_size = Q1 + Q2 + Q3 + Q4
    config.KV_size = 1024 # KV_size = Q1 + Q2 + Q3 + Q4
    # config.KV_size = [96,256,512,1024]  # KV_size = Q1 + Q2 + Q3 + Q4
...
def CIT():
    ...
    model = ChannelTransformer(a, vis, 56, channel_num=[64, 128, 320, 512])  # 224*224
    # model = ChannelTransformer(a, vis, 80, channel_num=[64, 128, 320, 512])    # 320* 320
...
class ConvBatchNorm3(nn.Module):
    """(convolution => [BN] => ReLU)"""
    ...
    def forward(self, x):
        ...
        out = self.norm(out)
        return self.activation(out)
class RAU_A(nn.Module):
    def __init__(self):
        super().__init__()

        self.conv_t1 = ConvBatchNorm1(in_channels=64, out_channels=32)
        self.conv_t2 = ConvBatchNorm1(in_channels=32, out_channels=32)

    def forward(self,t1,t2):
        t1 = self.conv_t1(t1)
        t1_ = torch.sigmoid(t1)
        t2 = self.conv_t2(t2)
        t2_ = torch.sigmoid(t2)
        o = 1-t1_
        out = t1_ * t1 + t2 * t2_ * o + t1
        return out

class RAU_B(nn.Module):
    def __init__(self):
        super().__init__()
...
class SBA(nn.Module):
    def __init__(self):
        ...
        self.raua = RAU_A()
        self.raub = RAU_B()
        self.conv = ConvBatchNorm3(in_channels=64, out_channels=32)
    def forward(self,a,b):
        #a是T1(64)  b是T2
        rau_a = self.raua(a,b)
        rau_b = self.raub(a,b)
        concat = torch.cat((rau_a, rau_b), dim=1)
        #print(concat.size())
